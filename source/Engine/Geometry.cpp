 ...
 * https://github.com/PfAndrey/supermariohd/blob/master/source/Geometry.cpp
 *
 */
...
Rect::Rect(const Vector &top_left, const Vector &size) : _left(top_left.x), _top(top_left.y),
...
{
    return abs(2 * (_left - other._left) + _width - other._width) < abs(_width + other._width) && abs(2 * (_top - other._top) + _height - other._height) < abs(_height + other._height);
}
